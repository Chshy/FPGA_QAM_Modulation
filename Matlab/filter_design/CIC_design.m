clc;clear;close all;
cic = dsp.CICInterpolator(8, 1, 2); %tau_p 创建CIC Interpolator对象
% 第一个参数 2 表示插值因子(InterpolationFactor)
% 第二个参数 1 表示滤波器组合部分的差分延迟(DifferentialDelay)
% 第三个参数 2 表示积分器和组合部分的数量(NumSections)
